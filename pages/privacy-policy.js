import Head from 'next/head';
import Breadcrumb from '../components/breadcrumb/Breadcrumb';
import Layout from '../components/layouts/Layout';

const PrivacyPolicy = () => {
    return (
        <Layout>
            <Head>
                <title>Privacy Policy || nextdev labs</title>
            </Head>

            <Breadcrumb title="Privacy Policy" current="Privacy Policy"/>

            <main className="page-wrappper">
                <div className="privacy-policy-area ax-section-gap">
                    <div className="container">
                        <div className="row">
                            <div className="col-lg-10 offset-lg-1">
                                <div className="content">
                                    <div className="inner">
                                        <div className="section-title">
                                            <h4 className="title">
                                                This Privacy policy was published on August 26th&sbquo;
                                                2024.
                                            </h4>
                                        </div>
                                        <h3>GDPR compliance</h3>
                                        <p className="subtitle-2 mb--40">
                                        

At NextDev Labs, we comply with the General Data Protection Regulation (GDPR) to ensure your personal data is handled securely and transparently. We collect, process, and store data only for necessary business operations, providing services, and enhancing user experience.
                                        </p>


                                        <h3>About NextDev Labs</h3>
                                        <p className="subtitle-2 mb--40">
                                        NextDev Labs is a web development agency that specializes in delivering digital solutions, including website and application development. For any inquiries regarding our privacy policy, please contact us at  <a legacyBehavior 
                                            href="#">info@nextdev.co.in</a>
                                        </p>

                                        <h3>When we collect personal data about you</h3>
                                        <ul className="mb--40">
                                            <li>When you visit our website and interact with our services.</li>
                                            <li>When you sign up for newsletters, promotions, or inquiries.</li>
                                            <li>When you request customer support or communicate with us.</li>
                                            <li>Through automated technologies like cookies and analytics tools.</li>
                                        </ul>

                                        <h3>Why we collect and use personal data</h3>
                                        <p className="subtitle-2">
                                        Provide, operate, and maintain our services.
                                        Improve our website functionality and user experience.
                                        Respond to customer inquiries and provide support.
                                        Comply with legal obligations and protect against fraud.
                                        </p>
{/* 
                                        <ul className="mb--40">
                                            <li>Nulla facilisi. Sed pulvinar nec purus eu sollicitudin. Quisque ut
                                                tempus quam&sbquo; in cursus eros.
                                            </li>
                                            <li>Fusce malesuada luctus velit eu tempor. Pellentesque habitant morbi
                                                tristique senectus et netus et.
                                            </li>
                                            <li>Pellentesque ornare nulla est&sbquo; non blandit sapien lacinia nec. Nulla ac
                                                velit id est mattis faucibus.
                                            </li>
                                            <li>Aliquam lacus nisi&sbquo; lobortis non diam eget&sbquo; malesuada bibendum justo.
                                                Praesent fringilla sagittis ex&sbquo; ac molestie ipsum ullamcorper a.
                                            </li>
                                            <li> Vestibulum nulla tortor&sbquo; aliquam at porta in&sbquo; hendrerit sed nibh.</li>
                                        </ul> */}

                                        <h3>Type of personal data collected</h3>
                                        <p className="subtitle-2 mb--40">
                                        Contact Information: Name, email address, phone number.
                                            Technical Data: IP address, browser type, and device information.
                                            Usage Data: Interactions with our website and services.
                                            Cookies & Tracking Data: Information for analytics and advertising.
                                        </p>

                                        <h3>Information we collect automatically</h3>
                                        <p className="subtitle-2 mb--40">
                                                                                IP address and browsing behavior.
                                        Device and operating system details.
                                        Referral sources and website navigation patterns.
                                        </p>

                                        <h3>The use of cookies and web beacons</h3>
                                        <p className="subtitle-2">
                                        We use cookies to:

                                            Personalize content and improve your browsing experience.
                                            Analyze website traffic using Google Analytics.
                                            Store user preferences for future visits.
                                        </p>
                                        <p className="subtitle-2">
                                            We may log information using digital images called web
                                            beacons on our Site or in our emails.
                                        </p>
                                        {/* <p className="subtitle-2 mb--40">
                                            This information is used to make our Site work
                                            more efficiently&sbquo; as well as to provide business and marketing information
                                            to the owners of the Site&sbquo; and to gather such personal data as browser type
                                            and operating system&sbquo; referring page&sbquo; path through site&sbquo; domain of ISP&sbquo; etc.
                                            for the purposes of understanding how visitors use our Site. Cookies and
                                            similar technologies help us tailor our Site to your personal needs&sbquo; as well
                                            as to detect and prevent security threats and abuse. If used alone&sbquo; cookies
                                            and web beacons do not personally identify you.
                                        </p> */}

                                        <h3>How long we keep your data</h3>
                                        <p className="subtitle-2">
                                        We retain personal data only for as long as necessary to fulfill our business purposes, comply with legal requirements, and resolve disputes.
                                        </p>

                                        <p className="subtitle-2 mb--40">
                                            We store personal data for as long as we find
                                            it necessary to fulfill the purpose for which the personal data was
                                            collected&sbquo; while also considering our need to answer your queries or resolve
                                            possible problems. This helps us to comply with legal requirements under
                                            applicable laws&sbquo; to attend to any legal claims/complaints&sbquo; and for
                                            safeguarding purposes.
                                        </p>

                                        <h3>Your rights to your personal data</h3>
                                        <p className="subtitle-2 mb--40">
                                        Request access to your personal data.
                                            Correct inaccurate or outdated information.
                                            Withdraw consent for marketing communications.
                                            Request data deletion under applicable laws.
                                        </p>

                                        {/* <h3>Hotjar’s privacy policy</h3>
                                        <p className="subtitle-2 mb--40">
                                            Pellentesque vestibulum venenatis iaculis.
                                            Aliquam viverra sodales ultrices. Quisque sed purus id massa consequat
                                            consectetur eu vel lorem. Maecenas lectus velit&sbquo; cursus quis orci non&sbquo;
                                            laoreet hendrerit mi. Nulla vitae ipsum fringilla&sbquo; placerat metus eu&sbquo;
                                            malesuada velit. Quisque viverra risus ex. Aenean commodo vestibulum
                                            efficitur. Nullam ligula orci&sbquo; aliquet sed luctus vel&sbquo; luctus vel dui. Sed
                                            pulvinar&sbquo; ipsum at mattis imperdiet&sbquo; diam augue tempor diam&sbquo; sed porttitor
                                            odio elit ut ante. In posuere mi at mi pellentesque ornare sit amet gravida
                                            nisi. Praesent ac blandit odio. Curabitur iaculis ante elit&sbquo; et imperdiet
                                            leo mollis at.
                                        </p> */}

                                        <h3>Changes to this Privacy Policy</h3>
                                        <p className="subtitle-2">
                                        NextDev Labs reserves the right to update this policy as needed. Changes will be posted on this page with the last update date.
                                        For any privacy-related concerns, contact us at info@nextdev.co.in.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </main>
        </Layout>
    );
};

export default PrivacyPolicy;
